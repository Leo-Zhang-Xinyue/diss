...
def build_data(data, dump_path, is_train=True, tolower=True):
    ...
    with open(dump_path, 'w', encoding='utf-8') as writer:
        for idx, sample in enumerate(data):
            ...
            input_ids =bc.encode(hypothesis + [' ||| '] + premise)
            type_ids = [0] * ( len(hypothesis) + 2) + [1] * (len(premise) + 1)
            features = {'uid': ids, 'label': label, 'token': input_ids, 'type_id': type_ids}
...
#             input_ids_1 = ['[CLS]'] + hypothesis_1 + ['[SEP]'] + premise + ['[SEP]']
...
#             input_ids_2 = ['[CLS]'] + hypothesis_2 + ['[SEP]'] + premise + ['[SEP]']
#             type_ids_2 = [0] * ( len(hypothesis_2) + 2) + [1] * (len(premise) + 1)
#             features = {'uid': ids, 'label': label, 'token': [input_ids_1, input_ids_2], 'type_id': [type_ids_1, type_ids_2], 'ruid':sample['ruid'], 'olabel':sample['olabel']}
#             writer.write('{}\n'.format(json.dumps(features)))

def build_data_single(data, dump_path):
